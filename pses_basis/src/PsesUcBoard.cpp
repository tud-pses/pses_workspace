...
PsesUcBoard::PsesUcBoard(const unsigned int baudRate,
                         const std::string deviceName)
    : baudRate(baudRate), deviceName(deviceName)
{
  connected = false;
...
void PsesUcBoard::initUcBoard(const unsigned int serialTimeout)
{
  ...
  std::string params;
  // US-Sensor Group
  sg = {Board::rangeSensorLeft, Board::rangeSensorFront,
        Board::rangeSensorRight};
  params = "~ALL=5";
...
const int PsesUcBoard::getId() const { return carID; }
...
void PsesUcBoard::setSteering(const int level)
{
  ...
  } while (answer.find(value) == -1 &&
           (ros::Time::now() - start).toSec() <= 0.1);
...
void PsesUcBoard::setMotor(const int level)
{
  ...
  if (level == 0)
  {
    valueStream << "F " << 0; // was -500 -> active braking, caused a
                              // malfunction where driving backwards was no
                              // longer possible
  }
  else if (level > 0)
  {
...
void PsesUcBoard::setSensorGroup(const Board::SensorGroup& sensors,
                                 const int numOfGroup,
    ...
    do
    {
      ...
      if (nextSensor < 0)
      {
        try
        {
          ...
          assignSensorValue(sensorMessage, sensorValue,
                            sensorGroups[groupID - 1][sensorCount]);
        }
        catch (std::exception& e)
        {
          throw UcBoardException(Board::SENSOR_PARSER_INVALID);
...
void PsesUcBoard::assignSensorValue(pses_basis::SensorData& data,
                                    const int value,
                                    const Board::SensorObject& sensor)
{
  switch (sensor)
  {
