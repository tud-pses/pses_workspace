...
/*	This empty constructor for ForwardKinematics requries to call setK() to
 *set distance
...
/*	This method calculates the momentaneous center of curvature for a
 *vehicle
...
/*	A utility function to remove values very close to zero, since numerical
 * sine
...
/*	Since ackermann driven cars always travel on circular lines (except when
 * they
 *  are driving straight forward), this method calculates the radius of those
 * circular
 ...
 *  matrix to rotate from S_i to S_0 (S means koordinates / S_0 are world
 * coordinates).
 *  It basically finds a way how the current coordinates need to be rotated to
 * fit
...
/*	Here we do not need to make assumptions about the cars orientation, with
 * the help
...
/*  A Method that calculates the homogenous transformation
 *  matrix to translate from S_i to S_0 (S means koordinates / S_0 are world
 * coordinates).
 *  It basically finds a way how the current coordinates need to be shifted to
 * fit
...
/*	Here we do not need to make assumptions about the cars position, with
 * the help
 *  of steering values. In this method only the Gyroscopes measurements are used
...
/* Here all of the functions from above are combined (except the ones that
 * calculate
 * with Gyroscope values). In this Method we take the old position as our new
 * base
 * coordinates and add our new driven path and rotation. Then transformation
 * matrices
...
/* Here all of the functions from above are combined (except the ones that don't
 * calculate
 * with Gyroscope values). In this Method we take the old position as our new
 * base
 * coordinates and add our new driven path and rotation. Then transformation
 ...
 * are used to transform the new coordinates in world coordinates.
 */
std::vector<double> ForwardKinematics::getUpdateWithGyro(double theta,
                                                         double deltaS)
{
  Eigen::Matrix4d* Ti = new Eigen::Matrix4d();
  *Ti = Eigen::Matrix4d::Identity();
